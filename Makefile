...
${CTO_BUILDALL}:
	$(eval CTO_TENSORFLOW_VERSION=$(shell echo $@ | cut -d_ -f 1))
	$(eval CTO_TENSORFLOW_TAG=${CTO_TENSORFLOW_VERSION}-py3)
	$(eval CTO_OPENCV_VERSION=$(shell echo $@ | cut -d_ -f 2))
	$(eval CTO_TAG=${CTO_TENSORFLOW_VERSION}_${CTO_OPENCV_VERSION}-${CTO_RELEASE})
	@echo ""; echo ""
	@echo "[*****] About to build tensorflow_opencv:${CTO_TAG}"
